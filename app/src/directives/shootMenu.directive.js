const shootMenuDirective = ($log, $timeout, $window) => {
    ...
    const link = ($scope, element, attrs) => {
        ...
        const trigger = options.find(({
            className
        }) => className.includes(triggerClass));
        if (!trigger) {
            $log.error(`ShootMenuDirective: No trigger found. Please make sure add "${triggerClass}" class.`);
            return;
        }
        const targets = options.filter((option, index) => options.indexOf(trigger) < index);
